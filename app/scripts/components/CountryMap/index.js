...
class CountryMap extends PureComponent {
	...
	render() {
		const { mapInfo } = this.props;
		const { key, center, zoom, filters, activeCity, isSidebarActive, activeFilter } = this.state;
			...
			<div className="CountryMap" style={{ height: "100vh", width: "100%" }}>
				<div className="filter">
					...
					<div className="filter__content">
						Наша миссия - упрощение покупки квартиры в новостройке. Точная и актуальная информация поможет
						вам выбрать именно то, что нужно. Мы убеждены, что каждый из нас заслуживает самый качественный
						сервис от застройщиков, банков и агентств недвижимости.
					</div>
				...
				>
					{mapInfo.data &&
						...
						mapInfo.data.coordinates
							.filter(({ category }) => {
								return category.indexOf(activeFilter) !== -1 || activeFilter === "все" ? true : false;
							})
							.map((city, key) => (
